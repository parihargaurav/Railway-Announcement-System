...
def textToSpeech(text, filename):
    mytext = str(text)
    language = 'hi'  # hindi announcement of train
...
# in pydub it starts from miliseconds
def generateSkeleton():
    audio = AudioSegment.from_mp3('railway.mp3')

    # 1 - Generate kripya dheyan dijiye
    start = 88000  # miliseconds hai time ok
    finish = 90200
...
def generateAnnouncement(filename):
    ...
    print(df)
    for index, item in df.iterrows():  # data frame function pandas ka data frame ka function hai
